...
curl -fsSL https://openrouter.ai/api/v1/model/anthropic/claude-opus-5 | jq '.data | {id, canonical_slug, context_length, pricing, supported_parameters, reasoning}'
```

Snapshot verified 2026-07-26 against the OpenRouter Models API. OpenCode uses the `openrouter/<model-id>` form; the Models API uses the provider/model ID without the `openrouter/` prefix.
...
| MoonshotAI Kimi K3 | `openrouter/moonshotai/kimi-k3` | 1.05M | Default enabled; `max`, `high`, `low`; default `max` | $3 / $15 |
...
| Anthropic Claude Opus 5 | `openrouter/anthropic/claude-opus-5` | 1M | `max`, `xhigh`, `high`, `medium`, `low`; default `high` | $5 / $25 |
| Anthropic Claude Opus 4.8 | `openrouter/anthropic/claude-opus-4.8` | 1M | `max`, `xhigh`, `high`, `medium`, `low`; default `high` | $5 / $25 |
...
The current catalog record reports text and image input with text output, a 1,048,576-token context window, tool use, structured output, and reasoning enabled by default. It supports `max`, `high`, and `low` efforts, with `max` as the default.
