...
Tracked `.serena/project.yml` configuration can follow a clone or worktree,
while local language-server caches do not. Index a fresh clone/worktree or a
repository with a major language/layout/dependency change:
...
If the failed Serena route omitted material evidence that a fallback found,
record it through the repository's retrieval-miss workflow when available.
