...
# Harness-injected user text in Claude Code transcripts: reminders, slash-command
# envelopes, local command echoes, and background-task notices. Not the user.
CLAUDE_SCAFFOLD = ("<system-reminder>", "<local-command-", "<task-notification>", "<bash-input>", "<bash-stdout>",
                   "Caveat: The messages below were generated by the user while running local commands")
# A slash command arrives as an envelope; the skill body it loads is a separate
# isMeta record. What the user typed is the command name and its arguments.
CLAUDE_COMMAND = re.compile(r"<command-name>\s*(?P<name>[^<]{1,200}?)\s*</command-name>")
CLAUDE_COMMAND_ARGS = re.compile(r"<command-args>(?P<args>.*?)</command-args>", re.S)
...
def header(path):
    """Authoritative session identity. `dialect` tells scan which records to expect."""
    try:
        with Path(path).open("rb") as handle:
            line = handle.readline(1024 * 1024)
            record = json.loads(line)
            if isinstance(record, dict) and isinstance(record.get("sessionId"), str) and "payload" not in record:
                return claude_header(Path(path), record, handle)
        ...
        if record.get("type") != "session_meta" or not isinstance(payload.get("id"), str) or not payload.get("cwd"):
            raise MemoryError("Transcript lacks authoritative session metadata")
        return {**payload, "dialect": CODEX}
    except (OSError, ValueError) as error:
        raise MemoryError("Cannot read native transcript metadata") from error


def claude_header(path, first, handle):
    """Claude Code has no header record: every record names its sessionId.

    Identity must agree with the file's location, so a transcript cannot claim
    another session: `<sessionId>.jsonl`, or a subagent sidechain under
    `<sessionId>/subagents/agent-*.jsonl`.
    """
    session_id = first["sessionId"]
    sidechain = path.parent.name == "subagents" and path.parent.parent.name == session_id and path.name.startswith("agent-")
    if path.stem != session_id and not sidechain:
        raise MemoryError("Transcript lacks authoritative session metadata")
    meta = {"id": session_id, "dialect": CLAUDE, "sidechain": sidechain}
    record = first
    for _ in range(200):  # Leading queue and attachment records carry no cwd.
        if isinstance(record, dict):
            if record.get("sessionId") not in (None, session_id):
                raise MemoryError("Transcript lacks authoritative session metadata")
            for source, target in (("timestamp", "timestamp"), ("cwd", "cwd")):
                if target not in meta and isinstance(record.get(source), str) and record[source]:
                    meta[target] = record[source]
        if "timestamp" in meta and "cwd" in meta:
            break
        line = handle.readline(1024 * 1024)
...
def events_for(session_id, agent, prefix, event_kind, role, identity, source_id, content, occurred_at, metadata):
    result = []
    for index, part in enumerate(chunks(content) if content else ("",)):
        event_id = prefix + digest([session_id, event_kind, role, identity, content, index])
        result.append({"event_id": event_id, "agent": agent, "native_session_id": session_id,
                       "source_record_id": str(source_id), "kind": event_kind, "role": role,
                       "content": part, "occurred_at": occurred_at,
                       "metadata": {**metadata, "capture_adapter": agent + "-v1", "chunk": index}})
    return result


def normalize_claude_record(record, session_id, source):
    """One Claude Code record holds several content blocks; each visible one is an event."""
    if not isinstance(record, dict):
        return [], ["malformed_record_skipped"]
    kind = record.get("type")
    attachment = record.get("attachment")
    if kind == "attachment" and isinstance(attachment, dict) and attachment.get("type") == "queued_command":
        # A message the user sent while a turn was running. It never becomes a
        # user record of its own, so this attachment is the only copy.
        text = visible_text(attachment.get("prompt", ""))
        if not text.strip() or text.lstrip().startswith(CLAUDE_SCAFFOLD):
            return [], []
        source_id = str(record.get("uuid") or source) + ":queued"
        return events_for(session_id, CLAUDE, "claude:v1:", "message", "user", source_id, source_id, text,
                          record.get("timestamp"), {"sent_mid_turn": True}), []
    # Other attachments, queue operations, titles, cost and mode records are
    # harness bookkeeping. isMeta marks text the harness wrote for the user.
    if kind not in {"user", "assistant"} or record.get("isMeta") is True:
        return [], []
    ...
    if not isinstance(blocks, list):
        return [], ["unsupported_payload"]
    base = {"agent_id": record["agentId"][:128]} if isinstance(record.get("agentId"), str) else {}
    record_id = record.get("uuid") or source
    compact = record.get("isCompactSummary") is True
    events, losses = [], []
    for index, block in enumerate(blocks):
        ...
        metadata = dict(base)
        if block_type in PRIVATE_TYPES:
            losses.append("private_record_excluded")
            continue
        if block_type == "text":
            text = visible_text(block)
            command = CLAUDE_COMMAND.search(text) if kind == "user" and "<command-name>" in text[:400] else None
            if command:
                typed = CLAUDE_COMMAND_ARGS.search(text)
                text = (command.group("name") + " " + (typed.group("args").strip() if typed else "")).strip()
                metadata["slash_command"] = True
            if kind == "user" and not compact and text.lstrip().startswith(CLAUDE_SCAFFOLD):
                continue
            event_kind, role = ("compaction", "assistant") if compact else ("message", kind)
            source_id = str(record_id) + ":" + str(index)
        elif block_type == "tool_use":
            name = str(block.get("name", "tool"))
            text = name + ": " + visible_text(block.get("input", ""))
            event_kind, role, metadata["tool"] = "tool_call", "assistant", name[:240]
            source_id = block.get("id") or str(record_id) + ":" + str(index)
        elif block_type == "tool_result":
            output = block.get("content")
            text = visible_text(output)
            event_kind, role = "tool_result", "tool"
            source_id = "result:" + str(block.get("tool_use_id") or str(record_id) + ":" + str(index))
            if output in ("", [], None):
                metadata["empty_result"] = True
            elif not text.strip():
                losses.append("private_record_excluded" if private_only(output) else "empty_or_unsupported_visible_content")
                continue
        else:
            losses.append("unsupported_content_block:" + str(block_type)[:60])
            continue
        if not text.strip() and not metadata.get("empty_result"):
            continue
        events.extend(events_for(session_id, CLAUDE, "claude:v1:", event_kind, role, source_id, source_id,
                                 text, record.get("timestamp"), metadata))
    return events, losses


NORMALIZERS = {CODEX: normalize_record, CLAUDE: normalize_claude_record}
...
def allowed_path(config, path):
    ...
    if not any(path.is_relative_to(root) for root in config.transcript_roots):
        raise MemoryError("Transcript must be a file inside a configured native transcript root")
    if not path.is_file():
        # Ephemeral utility runs (title or commit-message generation) fire
        # hooks without ever persisting a transcript.
...
def scan(path, session_id, cursor=None, baseline=None, dialect=CODEX):
    ...
    with Path(path).open("rb") as handle:
        ...
        for line in handle:
            ...
            try:
                ...
                continue
            visible, omitted = NORMALIZERS[dialect](record, session_id, "byte:" + str(start))
