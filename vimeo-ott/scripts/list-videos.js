...
const HELP = `
List Vimeo OTT videos.

Usage:
  node scripts/list-videos.js [options]

Options:
  --account <name>      Account name from VIMEO_OTT_ACCOUNTS
  --query <text>        Search term
  --plan <value>        Filter plan(s), comma separated or repeated in --query-json via get-resource.js
  --sort <mode>         alphabetical, newest, oldest, plays, finishes, or duration
...
async function main() {
  ...
  const body = await request(client, '/videos', {
    query: {
      query: args.query,
      plan: args.plan,
      sort: args.sort,
  ...
  outputJson({
    metadata: buildMetadata(accountName, '/videos', { page, perPage }),
    videos: summarizePage(body, 'videos'),
  });
}
