...
Version 1.2 adds Brave plugin reservations. Version 1.2.1 fixes Automatic detection when macOS launches a supported browser from a code-signing clone path. Older running status hosts/models can show unknown status while such a lease exists; reload them rather than deleting the lease. On this Mac the companion extension is installed in Chrome's Default, HOP, and Corcos.ca profiles, Comet, and Brave, with pinning and incognito access enabled. This is an installation record, not proof that those browsers/profiles are currently running or unchanged. Safari installation is intentionally excluded.
