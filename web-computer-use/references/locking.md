...
`--profile` is Chrome/Brave display-name metadata, never profile selection or proof. Add `--pinned` when the user explicitly chooses a browser/profile or project instructions require one. Inferred/default choices need not be pinned. `--pinned` plus any `--fallback` is rejected.

For a verified Brave plugin connection, use the same commands with `--browser brave --mode plugin` and its observed profile label, for example `--profile Work`. Chrome and Brave plugin leases reserve only their respective browser; hold separate leases when operating both. Profile labels do not partition access: Computer Use excludes plugin sessions across every profile of that browser. Comet and Safari plugin reservations are rejected.
...
The primary candidate uses `--mode`; `--fallback` candidates always use Computer Use, even for Brave. In this example, a free Chrome plugin connection gets shared Chrome access; otherwise Comet, Brave, or Safari requires exclusive browser and desktop access. Read the returned browser/mode before touching any browser. Omit candidates that cannot satisfy the task, or release an unusable acquired candidate before trying remaining ones with the **same original deadline**. A fallback to another browser clears the original profile label.

To prefer Brave's verified plugin connection during fallback, try suitable candidates in the skill's browser order using immediate `--wait 0` acquisitions, selecting `--browser brave --mode plugin` for that candidate. Pass one original `--until` deadline throughout, poll briefly after a complete unsuccessful pass, and report contention at that deadline. Do not first spend a full 600 seconds waiting for Chrome or desktop access. For an explicit browser/profile, omit all fallback attempts. Do not choose plugin mode for a flow requiring Keeper or user takeover.

For profile selection or authentication when already holding a Chrome or Brave plugin lease:
...
After verifying the plugin connects to that same browser and profile:
...
- An acquired/renewed result includes `lease` and `resources`. The lease contains its random token, owner, state, selected browser/mode, optional Chrome/Brave profile label, candidate list, and Unix-second timestamps. Plugin leases have `{"chrome":"shared"}` or `{"brave":"shared"}`; Computer Use adds `"desktop":"exclusive"` alongside the exclusive browser.
...
An upgrade from plugin to Computer Use relinquishes its shared reader and queues for exclusive browser plus desktop access. Two upgrading readers therefore cannot deadlock by retaining each other's required resource. Earlier queued exclusive requests block later readers/writers for the same browser. A queued writer does not reserve the desktop, so suitable work in another browser can still proceed. A Computer Use-to-plugin downgrade retains the existing Chrome or Brave ownership and profile label and releases the desktop atomically, without competing as a new reader.
...
Before using Brave plugin leases after upgrading from the Chrome-only helper, update the shared installed skill and any other helper copies in active use. Older helpers and already-running native status hosts reject Brave plugin records rather than ignoring them. Reload the companion status extension in each installed browser/profile to start its updated host and model; see [extension updates](extension.md#updating-an-existing-installation). No native lock files or active reservations need migration or deletion.
