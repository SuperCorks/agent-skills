...
description: Coordinate local web work in Chrome, Comet, Brave, and Safari on this Mac, including BrowserSkill (`bsk`) for Claude sessions, named profile connections, parallel Chrome/Brave plugin sessions, Keeper/passkey authentication through Computer Use, browser reservations, and busy-browser fallback. Use alongside the browser control skills, not for browser-profile migration or API-only tasks.
---
...
Use this coordination layer before operating a local browser. Prefer an applicable connector, API, or CLI for semantic work unless the user explicitly requests browser interaction.

**In Claude sessions (Claude Code, including inside T3 Code), use Tencent BrowserSkill for page work.** Claude has neither the `chrome:control-chrome` plugin nor native Computer Use there. Read the installed `browser-skill` skill and drive the user's logged-in Chrome or Brave through its `bsk` CLI and extension. It works in its own Agent Window and borrows a user tab only after the user confirms. Run `bsk status --json` first. An empty `browsers` list means no extension is connected; report that rather than installing anything. Use `bsk browsers` to pick the connection matching the profile chosen below, and pass its instance ID to `session start --browser`. Each extension-enabled Chrome profile connects separately, but connections carry no profile label and Brave also reports itself as `chrome`, distinguishable only by its older Chromium version. Verify the account from a non-secret signed-in label on a relevant page before account-specific work. A profile connects only while one of its windows is open. To open a missing Chrome profile, read only the display names under `profile.info_cache` in `~/Library/Application Support/Google/Chrome/Local State` to find its directory, run `open -na "Google Chrome" --args --profile-directory="<directory>"`, then recheck `bsk browsers` for a new connection after a few seconds. Treat a `bsk` session as plugin page work: hold a shared `--mode plugin` reservation for that browser, omit `--thread-id` (the helper accepts only Codex task UUIDs), and run `bsk session stop` before releasing. For steps that need the desktop (Keeper, passkeys, native dialogs, profile switching, Comet, or Safari), hand the step to the user with `bsk request-help` or delegate it to a Codex worker through `agent-orchestrator`. The plugin and Computer Use guidance below applies to Codex sessions.

When available, read the currently installed `chrome:control-chrome` skill before browser work; also read `computer-use:computer-use` before desktop interaction. Resolve their paths from the session's skill catalog; do not hardcode plugin versions or copy their APIs here. If a required skill/tool is unavailable, report that limitation and use only an available, authorized surface.
...
- **Passkeys do not work through the Chrome plugin.** When a site requires a passkey or security key, use **native Computer Use with Keeper** before clicking the site's authentication button and throughout the passkey flow. Use the native browser-app controls, not plugin-backed tab, DOM, Playwright, or page-JavaScript APIs, including when the plugin controls Brave. An exclusive Computer Use reservation does not turn a plugin-backed tab call into native Computer Use. Approve through Keeper's UI; if it requires user presence, release the reservation and hand off without switching to another authentication method.
